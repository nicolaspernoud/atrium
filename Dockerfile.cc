...
RUN apt update && apt install -y binutils-arm-linux-gnueabihf gcc-arm-linux-gnueabihf gcc-aarch64-linux-gnu libcap2-bin
...
RUN sed -i '/^#CUSTOM_ALLOCATOR/,/^#END_CUSTOM_ALLOCATOR/d' ./Cargo.toml && \
    sed -i '/^\/\/CUSTOM_ALLOCATOR/,/^\/\/END_CUSTOM_ALLOCATOR/d' ./src/main.rs
