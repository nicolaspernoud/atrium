use super::errors::{CryptoError, DavFileError};
...
impl CipherType {
    ...
    pub fn decrypted_size(&self, enc_size: u64) -> Result<u64, DavFileError> {
        ...
        if last_chunk_size < overhead {
            return Err(DavFileError::TruncatedChunk);
        }
        Ok((num_chunks - 1) * plain_chunk_size + (last_chunk_size - overhead))
    }
    ...
    pub fn create_cipher(
        ...
        key: &[u8; 32],
        nonce: &[u8],
    ) -> Result<Box<dyn Cipher>, DavFileError> {
        match self {
            CipherType::XChaCha20Poly1305_1M => {
                let aead = XChaCha20Poly1305::new(key.into());
                let stream_encryptor = StreamBE32::from_aead(
                    aead,
                    nonce.try_into().map_err(|_| DavFileError::InvalidLength)?,
                );
                Ok(Box::new(XChaCha20Poly1305Cipher {
                    stream_encryptor,
                    cipher_type: *self,
                }))
            }
...
impl Cipher for XChaCha20Poly1305Cipher {
    fn encrypt(
        ...
        self.stream_encryptor
            ...
            .map_err(|e| Box::new(CryptoError(e)) as Box<dyn Error + Send + Sync>)
    }
...
mod tests {
    ...
    fn test_decrypted_size() {
        ...
        assert_eq!(cipher_type.decrypted_size(0).unwrap(), 0);
        assert_eq!(
            cipher_type.decrypted_size(header_size + overhead).unwrap(),
            0
        );
        assert_eq!(
            cipher_type
                .decrypted_size(header_size + 3 * encrypted_chunk_size)
                .unwrap(),
            3 * plain_chunk_size
        );
        assert_eq!(
            cipher_type
                .decrypted_size(header_size + 3 * encrypted_chunk_size + overhead + 150)
                .unwrap(),
            3 * plain_chunk_size + 150
        );
        assert!(
            cipher_type
                .decrypted_size(header_size + overhead - 1)
                .is_err()
        );
    }
    ...
    fn test_encrypted_chunk_start() {
        ...
        assert_eq!(
            cipher_type.encrypted_chunk_start(plain_chunk_size - 1),
            header_size
        );
        assert_eq!(
            cipher_type.encrypted_chunk_start(plain_chunk_size),
            header_size + encrypted_chunk_size
        );
        assert_eq!(
            cipher_type.encrypted_chunk_start(2 * plain_chunk_size + 500),
            header_size + 2 * encrypted_chunk_size
        );
    }
}
