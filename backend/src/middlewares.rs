...
fn inject_security_headers_internal(resp: &mut Response, source: &str) -> Result<(), StatusCode> {
        ...
        .map(|h| h.to_owned())
    {
        ...
        None => {
            ...
            HeaderValue::from_str(&format!("default-src 'self' {source} https://unpkg.com https://fonts.gstatic.com; script-src 'self' {source} 'unsafe-inline' 'unsafe-eval' https://cdn.jsdelivr.net https://unpkg.com; style-src 'self' {source} 'unsafe-inline'; frame-src {source}; frame-ancestors {source}; img-src 'self' {source} blob:"))
        .map_err(|_| StatusCode::INTERNAL_SERVER_ERROR)?,);
        }
    }
    headers.insert(
